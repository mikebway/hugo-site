...
if (links) {
  Array.from(links).forEach(function(link) {
    ...
    isExternal = (url && typeof url == 'string' && url.startsWith('http')) &&
...
(function copyHeadingLink() {
  ...
  if (deeplinks) {
    document.body.addEventListener('click', function(event) {
      ...
      if (target.classList.contains(deeplink) ||
...
(function copyLinkToShare() {
  let copy, copied, excerpt, isCopyIcon, isInExcerpt, link, page, postCopy,
      postLink, target;
  copy = 'copy';
  copied = 'copy_done';
  excerpt = 'excerpt';
  ...
  page.addEventListener('click', function(event) {
    ...
    if (isCopyIcon) {
      if (isInExcerpt) {
        ...
        link =
            containsClass(link, postLink) ? elemAttribute(link, 'href') : false;
      } else {
...
function currentMode() {
  ...
  let mode =
      getComputedStyle(doc).getPropertyValue(key).replace(/\"/g, '').trim();
